...
\begin{tikzpicture}[
  ...
  every node/.style={rectangle, draw, rounded corners, align=center, font=\small, minimum width=25mm},
  level distance=15mm,
  grow=east,
  top level/.style={sibling distance=0.18\textwidth}, % top-level nodes evenly spaced
  second level/.style={sibling distance=20mm}          % child nodes spacing
]

\node {Toolbar}
    child [top level] { node {File}
        child [second level] { node {Open} }
        child [second level] { node {Exit} }
    }
    child [top level] { node {View}
        child [second level] { node {Close all tabs} }
        child [second level] { node {Expand Treeview} }
    }
    child [top level] { node {Jar}
        child [second level] { node {Open Manifest} }
    }
    child [top level] { node {UML}
        child [second level] { node {Open diagram} }
        child [second level] { node {Save image} }
    }
    child [top level] { node {Help}
        child [second level] { node {About} }
    };
